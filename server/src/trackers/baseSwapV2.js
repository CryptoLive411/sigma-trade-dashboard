...
const DEFAULT_FACTORY = '0xFDa619b6d20975be80A10332cD39b9a4b0FAa8BB'
const DEFAULT_ROUTER = '0x327df1e6de05895d2ab08513aadd9313fe505d86'

function create(ctx) {
  const id = 'baseswapv2-' + uuidv4()
  ...
  const state = {
    ...
    type: 'dex',
    enabled: true,
    priority: ctx.runtime?.priorities?.BaseSwapV2 ?? 0,
    include: [],
    blacklist: [],
      ...
      try {
        ...
        for (const log of receipt.logs) {
          ...
          try {
            ...
            // Early reservation enforcing limits (only if tracker is enabled)
            if (state.enabled) {
              try {
                ...
                if (!res.ok) {
                  ...
                  if (!options || !options.wantReason) state.io.emit('trade:skip', { dex: 'BaseSwapV2', reason: lastReason, tokenOut: token0 === WETH ? token1 : token0 })
                  continue
                }
              } catch (_) { }
            }
        ...
        if (options && options.wantReason) return { reason: lastReason || 'no-match' }
      } catch (e) { logger.error(e, 'BaseSwapV2 detect error') }
      return null
    },
  ...
  async function buyAndMonitor({ token0, token1, pair, receipt, router, state, WETH, ROUTER }) {
    ...
    try {
      ...
      trade = await newTrade(state.redis, { trackerId: state.id, trackerName: state.name, dex: 'BaseSwapV2', token: tokenOut, pool: pair })
    } catch (_) { }
    let receiptBuy
    try {
      receiptBuy = await state.queue.enqueue({ signer, buildTx, label: 'baseswapv2-buy', trade: trade?.id, action: 'buy' })
    } catch (e) {
    ...
    state.io.emit('trade:buy', { dex: 'BaseSwapV2', tokenOut, amountIn: trading.buyEthAmount, boughtAmount, pair })

    const started = Date.now()
    let sold = false
    let prevPnl = null
    const buyValueEth = ethers.utils.formatUnits(amountIn, 18)
    ...
    while (!sold) {
      ...
      try {
        ...
        // Include manual exposure baseline in denominator
        let expWei = '0'
        ...
        const inEth = Math.max(1e-12, parseFloat(buyValueEth) + expEth)
        const pnl = (outEth - inEth) / inEth
        state.io.emit('trade:monitor', { dex: 'BaseSwapV2', tokenOut, outEth, pnl, trackerId: state.id, trackerName: state.name })
        // Manual mode: only stoploss applies when enabled; otherwise profit/loss/time
        let manual = null
        ...
        if (shouldSell) {
          ...
          if (allowance.lt(bal)) {
            ...
            await state.queue.enqueue({ signer, buildTx: buildApprove, label: 'baseswapv2-approve-sell' })
          ...
          const receiptSell = await state.queue.enqueue({ signer, buildTx: buildTx2, label: 'baseswapv2-sell', trade: trade?.id, action: 'sell' })
          try {
            const rx = await computeEthReceivedForTx(state.provider, me, receiptSell)
            const pnlWei = rx.sub(amountIn)
      ...
      } catch (e) { logger.warn(e, 'BaseSwapV2 monitor transient error') }
    }
  }
...
function attachManual(state) {
  ...
  async function ensureManualMonitor(token) {
    ...
    try {
      while (true) {
        ...
        let outEth = null
        try { const amounts = await routerRO.getAmountsOut(bal, [token, WETH]); if (amounts && amounts[1]) outEth = Number(ethers.utils.formatUnits(amounts[1], 18)) } catch (_) { }
        let inEth = 0
        ...
        state.io.emit('trade:monitor', { dex: 'BaseSwapV2', tokenOut: token, outEth, pnl, trackerId: state.id, trackerName: state.name, startedAt: started, manual: true })
      }
    } finally { state._manualMonitors.delete(token.toLowerCase()) }
  }
  state.manualBuy = async ({ token, amountEth }) => {
    ...
    let tradeId = null
    try { const { ensureOpenTrade } = require('../lib/trades'); const tr = await ensureOpenTrade(state.redis, { trackerId: state.id, trackerName: state.name, dex: 'BaseSwapV2', token }); tradeId = tr?.id || null } catch (_) { }
    const receipt = await state.queue.enqueue({ signer, buildTx, label: 'baseswapv2-manual-buy', trade: tradeId, action: 'buy', priority: true, resolveOnSent: true })
    try {
      if (tradeId) {
    ...
    state.io.emit('trade:buy', { dex: 'BaseSwapV2', tokenOut: token, amountIn: String(amountEth) })
  ...
  state.manualSell = async ({ token, amountPct }) => {
    ...
    if (!alreadyApproved) {
      ...
      await state.queue.enqueue({ signer, buildTx: buildApprove, label: 'baseswapv2-manual-approve', priority: true, resolveOnSent: true })
    ...
    const receipt = await state.queue.enqueue({ signer, buildTx: buildSell, label: 'baseswapv2-manual-sell', trade: tradeId, action: 'sell', priority: true, resolveOnSent: true })
    ...
    state.io.emit('trade:sell', { dex: 'BaseSwapV2', tokenOut: token })
    // If full manual sell, close the open trade and free the slot
    try {
