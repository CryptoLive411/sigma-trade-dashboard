...
// Zora Launchpad (Uniswap V4 flavor)
// Allows only a specific hooks address: 0xff74Be9D3596eA7a33BB4983DD7906fB34135040
const ALLOWED_HOOK = '0xff74Be9D3596eA7a33BB4983DD7906fB34135040'

function create(ctx) {
  const base = baseUniv4.create(ctx)
  base.name = 'Zora Launchpad'
  base.id = 'zora-' + uuidv4()
  ...
  base.detect = async (receipt, options) => {
          ...
          if (base.enabled) {
            try {
              ...
              if (!res.ok) {
                ...
                if (!options || !options.wantReason) base.io.emit('trade:skip', { dex: 'ZoraV4', reason })
    ...
    } catch (e) { logger.error(e, 'zora detect error') }
